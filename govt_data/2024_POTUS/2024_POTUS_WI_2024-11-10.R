...
src_url <- "https://elections.wi.gov/sites/default/files/documents/Election%20Night%20Unofficial%20Results%20Reporting.xlsx"
chart_caption = paste0("Chart: X:@bdill threads:@wbdill\n", src_url)
state <- "Wisconsin"

raw <- read_csv("D:/opendata/PresidentialElection2024/WI/Wisconsin_2024_potus_unofficial.csv") 
raw[is.na(raw)] <- 0

#str_replace_all(raw$county, " COUNTY", "")

potus_wide <- raw %>% 
  #pivot_wider(names_from = party, values_from = votes) %>% 
  mutate(ind_votes = ind1 + ind2 + ind3 + ind4 + ind5 + ind6 + ind7  #sum(ind1, ind2, ind3, ind4, ind5, ind6, ind7, na.rm=TRUE)
  ...
  mutate(  dem_pct = round(dem_votes/total_votes, digits = 4)
           ...
           , county = str_replace_all(raw$county, " COUNTY", "")
  )

...
twain_books <- gutenberg_metadata %>%
  filter(str_detect(author, "Twain,"), has_text == TRUE) %>%
...
saveRDS(dickens_book_data, "C:/Data/R/dickens_book_data.rds")
...
ggplot(twain_sentiment_by_index, aes(index, net_sentiment, fill = title)) +
